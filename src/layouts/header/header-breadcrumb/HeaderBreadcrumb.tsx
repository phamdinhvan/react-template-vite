...
const HeaderBreadcrumb: React.FC<any> = ({ menuData }) => {
  ...
  const menuItems = useMemo(() => getBreadcrumbName(location.pathname), [location.pathname]);
      ...
      <div className='tw-flex tw-items-center tw-relative tw-z-[1003]'>
        <Breadcrumbs>
          {menuItems?.map((item: any, index: number) => {
            return (
              <span key={index} className='tw-et-text-heading-24 tw-text-ink-light tw-normal-case'>
                {item?.text}
              </span>
            );
          })}
